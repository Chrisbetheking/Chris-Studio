...
import { classifyPrivacy, isLocalProviderId, privacyRouteLabel } from '../features/privacy/contentClassifier';
...
export function WorkspaceScreen({
  ...
  const privacyAssessment = useMemo(() => classifyPrivacy({
    text: [prompt, ...attachments.map((attachment) => attachment.content)].join('\n\n').slice(0, 600_000),
    paths: attachments.map((attachment) => attachment.name),
  ...
  const submit = () => {
    ...
    try {
      ...
      const remoteProvider = !isLocalProviderId(provider.providerId);
      const warnings: string[] = [];
      if (requestScan.riskLevel === 'critical' && settings.blockCriticalSends) {
        warnings.push(copy(language,
          'Critical values were detected. Only the redacted payload will be used.',
          '检测到严重风险内容，系统只会使用脱敏后的内容。'));
      } else if (requestScan.riskLevel === 'high' || requestScan.riskLevel === 'medium') {
        warnings.push(copy(language,
          `Chris Studio found ${requestScan.findings.length} sensitive item(s); the payload will be redacted.`,
          `Chris Studio 检测到 ${requestScan.findings.length} 处敏感内容，发送前会进行脱敏。`));
      }
      if (remoteProvider && privacyAssessment.route !== 'remote-ok') {
        warnings.push(copy(language,
          `Content-aware privacy routing recommends ${privacyAssessment.route === 'local-only' ? 'local processing' : 'manual review'} (score ${privacyAssessment.score}/100). Continue with ${provider.displayName}?`,
          `内容感知隐私路由建议${privacyAssessment.route === 'local-only' ? '仅本地处理' : '人工确认'}（评分 ${privacyAssessment.score}/100）。仍使用 ${provider.displayName} 吗？`));
      }
      ...
      <section className="ua-main">
          ...
          {!timeline.length && (
            <div className="ua-empty">
              ...
              <div className="ua-capability-grid">
                ...
                <span>Content-aware privacy routing</span>
                <span>Structured multi-model comparison</span>
              </div>
            </div>
          )}
        ...
        <footer className="ua-composer-wrap">
          ...
          {(prompt.trim() || attachments.length) ? (
            <div className={`ua-privacy-route route-${privacyAssessment.route}`}>
              <strong>{copy(language, 'Privacy preflight', '隐私预检')}</strong>
              <span>{privacyRouteLabel(privacyAssessment.route, language)} · {privacyAssessment.score}/100</span>
              {privacyAssessment.reasons.length ? <small>{privacyAssessment.reasons.slice(0, 3).join(' · ')}</small> : null}
            </div>
          ) : null}
          {attachments.length ? (
