...
export function UpdatesScreen({ language }: { language: Language }) {
  ...
  return <main className="modern-page updates-page">
    ...
    <section className="update-hero"><div className={`update-orb ${info?.updateAvailable ? 'available' : ''}`}><Icon name={info?.updateAvailable ? 'download' : 'check'} size={30} /></div><span className="section-kicker">TOKENFENCE STUDIO</span><h2>{info?.updateAvailable ? copy(language, `Version ${info.latestVersion} is available`, `发现新版本 ${info.latestVersion}`) : copy(language, 'You are on the latest published version', '当前已是最新发布版本')}</h2><p>{info?.errorMessage ?? copy(language, `Installed: ${info?.currentVersion ?? '1.7.1'} · Latest: ${info?.latestVersion ?? 'checking'}`, `已安装：${info?.currentVersion ?? '1.7.1'} · 最新：${info?.latestVersion ?? '检查中'}`)}</p>{info?.releaseUrl && <button className="button primary" onClick={() => void openExternal(info.releaseUrl!)}><Icon name="external" />{copy(language, 'Open GitHub Release', '打开 GitHub Release')}</button>}</section>
