...
async function runAgent(item: QueueItem, controller: AbortController): Promise<void> {
  ...
  const contextMessages: Pick<ChatMessage, 'role' | 'content'>[] = [
    { role: 'system', content: unifiedAgentSystemPrompt(loadProjectRoot(), item.privacy) },
    ...limitedHistory(conversation),
  ];
...
${attachmentContext}` });
  ...
  let latestAccessibility: { app?: string; maxIndex: number; elements: Array<{ index: number; role: string; title: string; enabled: boolean; actions: string[] }> } | undefined;
