...
export async function activateAccessibility(index: number, app: string | undefined, expectedRole: string, expectedTitle: string, confirmed: boolean): Promise<AccessibilityActionResult> {
  if (!isDesktopRuntime()) {
  ...
  return invoke<AccessibilityActionResult>('computer_activate_accessibility', {
    ...
    expectedTitle: expectedTitle.trim() || null,
    confirmed,
  });
}
