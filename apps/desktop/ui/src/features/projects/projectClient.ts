...
export async function chooseProjectFolder(): Promise<ProjectWorkspace | null> {
  ...
  return invoke<ProjectWorkspace | null>('project_choose_folder');
}
...
export async function rollbackProjectChangeSession(sessionId: string, paths: string[], confirmed: boolean): Promise<ProjectChangeSessionResult> {
  return invoke<ProjectChangeSessionResult>('project_rollback_change_session', { sessionId, paths, confirmed });
}
export async function acceptProjectChangeSession(sessionId: string, paths: string[], confirmed: boolean): Promise<ProjectChangeSessionResult> {
  return invoke<ProjectChangeSessionResult>('project_accept_change_session', { sessionId, paths, confirmed });
}
export async function createGitBranch(branch: string, confirmed: boolean): Promise<ProjectCommandResult> {
  return invoke<ProjectCommandResult>('project_git_create_branch', { branch, confirmed });
}
export async function commitProjectChanges(message: string, paths: string[], confirmed: boolean): Promise<ProjectCommandResult> {
  return invoke<ProjectCommandResult>('project_git_commit', { message, paths, confirmed });
}
export async function pushGitBranch(branch: string, confirmed: boolean): Promise<ProjectCommandResult> {
  return invoke<ProjectCommandResult>('project_git_push', { branch, confirmed });
}
