...
const SCOPED_COMMIT = `fn latest_reviewed_commit_paths(root: &Path) -> Result<HashSet<String>, String> {
    ...
    for session_id in session_ids {
        let (_, _, manifest) = match load_project_change_manifest(root, &session_id) {
            Ok(value) => value,
            Err(_) => continue,
        };
        if !matches!(manifest.status.as_str(), "applied" | "partially-accepted" | "accepted") { continue; }
        let paths = manifest.files.iter()
...
fn project_git_commit(message: String, paths: Vec<String>, confirmed: bool, state: State<'_, AppState>) -> ProjectCommandResult {
    ...
    let allowed = match latest_reviewed_commit_paths(&root) { Ok(paths) => paths, Err(error) => return failure(error) };
...
function patchMain() {
  patchText('apps/desktop/src-tauri/src/main.rs', (source) => {
    ...
    if (!next.includes('fn latest_reviewed_commit_paths(root: &Path)')) {
...
function patchCoreTests() {
  patchText('apps/desktop/ui/scripts/run-core-tests.cjs', (source) => {
    if (source.includes('scripts/v2-4-unified-agent-test.cjs')) return source;
    return replaceRequired(source, '  "scripts/v2-2-final-closeout-test.cjs",', '  "scripts/v2-2-final-closeout-test.cjs",\n  "scripts/v2-4-unified-agent-test.cjs",', 'Unified Agent core test');
  });
}
...
function verify() {
  ...
  if (!workflow.includes('default: v2.4.0-alpha.1')) throw new Error('Release workflow does not default to v2.4.0-alpha.1.');
...
verify();
console.log('CHRIS_STUDIO_V2_4_ALPHA1_OVERLAY_READY');
